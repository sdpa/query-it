...
export const DatabaseForm = () => {
  ...
  useEffect(() => {
    setCredentials({
      type: dbType,
      ...Object.fromEntries(
        Object.entries(DATABASE_CONFIGS[dbType] || {}).map(([key, config]) => [
          key,
          (config as FieldConfig).defaultValue
        ])
      )
    })
  }, [dbType])
  ...
  const getFields = () => {
    ...
    return Object.entries(fieldConfig).map(([fieldName, config]) => renderField(fieldName, config))
  }

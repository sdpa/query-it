...
export const DatabaseForm = () => {
  ...
  // Render form fields based on the selected database type
  const renderFormFields = () => {
    const fieldConfig = DATABASE_CONFIGS[dbType] || {};
    
    return Object.entries(fieldConfig).map(([fieldName, config]) => {
      // Skip rendering if dbType is not yet set
      if (!dbType) return null;
      
      // If field is optional and marked as such in the UI, handle accordingly
      if (config.optional) {

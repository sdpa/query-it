  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
      ...
      if (input.toLowerCase().includes("users") && input.toLowerCase().includes("email")) {
        response = "Here's a query to get all users with their email addresses:\n\n```sql\nSELECT id, name, email FROM public.users;\n```";
      } else if (input.toLowerCase().includes("product") && input.toLowerCase().includes("price")) {
        response = "Here's a query to get all products sorted by price:\n\n```sql\nSELECT id, name, price, category FROM public.products\nORDER BY price DESC;\n```";
      } else if (input.toLowerCase().includes("join") || (input.toLowerCase().includes("order") && input.toLowerCase().includes("user"))) {
        response = "Here's a query to join orders with users:\n\n```sql\nSELECT o.id, u.name, o.total, o.status\nFROM public.orders o\nJOIN public.users u ON o.user_id = u.id\nORDER BY o.created_at DESC;\n```";
      } else {
        response = "I'll help you write a SQL query for that. Could you provide more details about the tables and fields you want to query?";
      }
                ...
                <div className="whitespace-pre-wrap">
                  {message.content.split("```").map((part, i) => {
                    ...
                    if (i % 2 === 1 && part.startsWith("sql\n")) {
                      ...
                      return (
                        <div key={i} className="bg-black/10 p-2 rounded my-2 font-mono text-xs overflow-x-auto">
                          {code}
                        </div>
                      );
                    }
